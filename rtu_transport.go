...
func (tt *rtuTransport) readMBAPFrame() (p *pdu, err error) {
	var rxbuf []byte
	var bytesNeeded int
	var byteCount int
	// set an i/o deadline on the link
	err = tt.link.SetDeadline(time.Now().Add(tt.timeout))
	var unitId uint8
	var crc crc
	// read the MBAP header
	rxbuf = make([]byte, maxRTUFrameLength)

	byteCount, err = io.ReadFull(tt.link, rxbuf[0:8])
	if (byteCount > 0 || err == nil) && byteCount != 8 {
		err = ErrShortFrame
		return
	}
	unitId = rxbuf[0]
	...
	if rxbuf[1] == 0x10 {
		bytesNeeded = int(rxbuf[5])*2 + 1

	}

	//fmt.Printf("bytesNeeded:%v\n", bytesNeeded)
	// we need to read 2 additional bytes of CRC after the payload

	if bytesNeeded > 0 {
		byteCount, err = io.ReadFull(tt.link, rxbuf[8:8+bytesNeeded])
		if err != nil && err != io.ErrUnexpectedEOF {
			return
		}
		if byteCount != bytesNeeded {
			tt.logger.Warningf("expected %v bytes, received %v", bytesNeeded, byteCount)
			err = ErrShortFrame
			return
		}
	...
	crc.init()
	crc.add(rxbuf[0 : 8+bytesNeeded-2])

	// compare CRC values
	if !crc.isEqual(rxbuf[8+bytesNeeded-2], rxbuf[8+bytesNeeded-1]) {
		err = ErrBadCRC
		return
	}
	fmt.Printf("read:%v\n", rxbuf)
	...
	p = &pdu{
		...
		payload:      rxbuf[2 : 8+bytesNeeded-2],
	}
...
func (rt *rtuTransport) ReadRequest() (req *pdu, err error) {
	...
	// err = rt.socket.SetDeadline(time.Now().Add(tt.timeout))
	// if err != nil {
	// 	return
	// }
